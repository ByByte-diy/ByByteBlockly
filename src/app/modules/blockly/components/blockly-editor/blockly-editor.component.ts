...
export class BlocklyEditorComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  public generateCode(language: string = 'Arduino'): string {
    ...
    try {
      ...
      if (BlocklyGlobal && BlocklyGlobal.Arduino && 
...
Saved data will be cleared after pressing OK.
  ...
  public async exportProject(): Promise<void> {
    try {
      ...
      const projectXml = this.storage.exportWorkspaceToBBB(this.workspace, settings);
    ...
    input.onchange = async (e: any) => {
      ...
      reader.onload = async (event: any) => {
        try {
          ...
          // Load workspace
          this.storage.loadWorkspaceFromXml(this.workspace, parsed.workspaceXml);
