...
@Component({
  ...
  template: `
    <div class="level-switcher" role="group" [attr.aria-label]="label">
      <span class="level-switcher__label">{{ label }}</span>
...
export class LevelSwitcherComponent implements OnInit, OnDestroy {
  ...
  protected readonly options: LevelOption[] = [
    { level: BlockLevelE.BEGINNER, labelKey: "LEVEL_BEGINNER" as keyof typeof Blockly.Msg },
    { level: BlockLevelE.INTERMEDIATE, labelKey: "LEVEL_MIDDLE" as keyof typeof Blockly.Msg },
    { level: BlockLevelE.ADVANCED, labelKey: "LEVEL_PRO" as keyof typeof Blockly.Msg },
  ];
  ...
  private refreshLabels(): void {
    this.label =
      (Blockly.Msg["LEVEL_LABEL"] as string) || "Level:";
  }
}
