...
import { IStorageAdapter, IAdvancedStorageAdapter } from './lib/storage-adapter.interface';
...
export class WorkspaceStorageService {
  ...
  private async initializeAdapter(): Promise<IStorageAdapter> {
    ...
    console.log(`💾 Storage initialized: ${this.adapter.getType()} on ${this.adapter.getPlatform()}`);
    return this.adapter;
  }
   ...
   */
  async saveWorkspace(workspace: Blockly.WorkspaceSvg, debounce = true): Promise<void> {
    ...
    const doSave = async () => {
      try {
        ...
        await this.adapter.setItem(STORAGE_KEYS.LAST_SAVED, Date.now().toString());
  ...
  async importData(data: IStorageData): Promise<boolean> {
    try {
      ...
      if (data.settings) {
        await this.adapter.setItem(STORAGE_KEYS.SETTINGS, JSON.stringify(data.settings));
      }
      await this.adapter.setItem(STORAGE_KEYS.LAST_SAVED, data.timestamp.toString());
   ...
   */
  exportWorkspaceToBBB(workspace: Blockly.WorkspaceSvg, settings: IAppSettings): string {
    // Get workspace XML
    const xml = Blockly.Xml.workspaceToDom(workspace);
    const xmlText = Blockly.Xml.domToText(xml);
...
${xmlText}
  ...
  parseProjectFile(xmlContent: string): { 
    workspaceXml: string; 
    metadata?: { board?: string; port?: string; version?: string; timestamp?: string }
  } {
    // Parse XML
    ...
    if (bbbProject) {
      ...
      if (metadataNode) {
        const board = metadataNode.querySelector('board')?.textContent || undefined;
        const port = metadataNode.querySelector('port')?.textContent || undefined;
   ...
   */
  loadWorkspaceFromXml(workspace: Blockly.WorkspaceSvg, xmlString: string): void {
  ...
  private async setStorageVersion(version: string = STORAGE_VERSION): Promise<void> {
